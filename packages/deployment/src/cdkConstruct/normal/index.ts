...
export default class NextJSStack extends Stack {
  ...
  constructor(scope: Construct, id: string, private nextAppRoot: string, props?: StackProps) {
    ...
    const defaultLambdaURL = this.defaultLambda.addFunctionUrl();
    ...
    this.nextCloudfront = new Distribution(this, 'NextJSDistribution', {
      ...
      defaultBehavior: {
        // Must use
        origin: new HttpOrigin(Fn.select(2, Fn.split('/', defaultLambdaURL.url))),
    ...
    defaultLambdaURL.grantInvokeUrl(
      new OriginAccessIdentity(this, 'My CF Identity').grantPrincipal,
    );
